...
return {
  ...
  -- SQUARE ROOT
  s(
    { trig = "([^%\\])sqt", wordTrig = false, regTrig = true, snippetType = "autosnippet" },
    fmta("<>\\sqrt{<>}", {
      f(function(_, snip)
        return snip.captures[1]
      end),
  ...
  s(
    { trig = "([^%a])pmm", wordTrig = false, regTrig = true, snippetType = "autosnippet" },
    fmta("<>\\prod_{<>}^{<>}", {
      f(function(_, snip)
        return snip.captures[1]
      end),
  ...
  -- LEADS TO (PATH)
  s(
    { trig = "([^%a])lto", wordTrig = false, regTrig = true, snippetType = "autosnippet" },
    fmta("<>\\overset{<>}{\\leadsto}", {
      f(function(_, snip)
        return snip.captures[1]
      end),
      d(1, get_visual),
  ...
  s({ trig = "ixx", snippetType = "autosnippet" }, {
    t("\\exists "),
  }, { condition = tex.in_mathzone }),
  -- NATURAL LOG
